...
<body>
	...
	function findSR01() {
		...
		gfn_ajaxRest("sr01/find/" + $("#likeSelect").val() + "/" + $("#findText").val(), "GET" , function(data) {	
			var html = "";
			
			$.each(data.list, function(index, item) {
				html += '<tr>';
	            html += '<td scope="col" width="50">' + item.seq + '</td>';
	            html += '<td scope="col" width="50"><a href="/sr01/show_detail/'+
	            item.seq +'">' + 
	            item.title + '</a></td>';
	...
	function showSR01List(nowPage) {
		...
		gfn_ajaxRest("sr01/" + page, "GET" , function(data) {
			var html = "";
			$.each(data.list, function(index, item) {
				html += '<tr>';
	            html += '<td scope="col" width="50">' + item.seq + '</td>';
 ...
 <jsp:include page="banner.jsp" /> 
	...
	<button id="btn_findText_button" type="button" >게시물 검색</button>
		...
		<div id="pagenation">
			...
			<form id="write_form" action="/sr01/insertPage" method="get">
